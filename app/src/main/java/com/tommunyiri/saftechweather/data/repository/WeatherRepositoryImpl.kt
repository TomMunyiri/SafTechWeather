    ...
    override suspend fun storeForecastData(forecasts: List<Forecastday>) =
        withContext(ioDispatcher) {
            ...
            mapper.transformToDto(forecasts).let { listOfDbForecast ->
                listOfDbForecast.forEach { dbForecast ->
                    dbForecast.modifiedAt = dateFormat.format(Date(System.currentTimeMillis()))
                    localDataSource.saveForecastWeather(dbForecast)
                }
            }
        }
